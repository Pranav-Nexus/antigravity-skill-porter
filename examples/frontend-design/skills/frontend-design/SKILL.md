...
1. a warm cream background (near #F4F1EA) with a high-contrast serif display and a terracotta or warm-clay accent (often near #D97757 — Claude/Anthropic's warm-clay accent, so on a user's brief it reads as an AI tell);
...
## Antigravity Environment Synergy

When developing frontend designs inside Google Antigravity:
1. **Modern Web Guidance**: Pair this skill with Antigravity's `modern-web-guidance` skill for modern CSS patterns (`container queries`, `subgrid`, `@property`, `color-mix()`, View Transitions).
2. **Visual Inspection**: Use Chrome DevTools MCP or `view_file` to review rendered layout snapshots, inspect the a11y tree, and audit color contrast.
3. **Design Token Artifacts**: Output proposed design token systems (hex palettes, typography scales, layout wireframes) as structured Antigravity markdown artifacts (`<appDataDir>\brain\<conversation-id>\design_tokens.md`) via `write_to_file`.
